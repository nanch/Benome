...
function Colors(contexts, options) {
    options = options || {};
...
_.extend(Colors.prototype, {
    ...
    getColor: function(contextID, fresh, baseLightnessAdjust, noCache) {
        ...
        var rootContext = this.contexts.get(this.rootContextID);
        if (!rootContext) {
            return '#ccc';
        }
        ...
        var baseContextID = this.contexts.getBaseContext(contextID),
        ...
        function depthColor(contextID) {
            var parentContext = _this.contexts.get(contextID),
