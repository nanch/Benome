    ...
    setFilterLevel: function(filterLevel, options) {
        ...
        if (newFilterLevel != this.filterLevel) {
            ...
            if (this.config.setBackgroundFilterLevel) {
                var baseBGColor = $.Color(this.G.bgColor),
                    transitionDistance = newFilterLevel / this.config.numDetailLevels,
                    filterColor = baseBGColor.transition($.Color('#888'), transitionDistance);

                this.$container.css({
                    'background-color': filterColor.toHexString()
                });
