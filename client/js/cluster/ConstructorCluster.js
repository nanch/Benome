...
function ConstructorCluster($container, clusterOptions, constructContextDef) {
    ...
    var cluster = new Cluster('ConstructorCluster-1234', this, {$el: this.$container});
...
_.extend(ConstructorCluster.prototype, BaseClusterController.prototype, {
    ...
    addContext: function(contextDef, parentID, parentContext, parentSurfaceView) {
        ...
        if ((parentContext.get('nodeType') == 'AttributeChoice') && (parentSurfaceView.getValue() != 'AttributeGroup')) {
            return;
        }
    ...
    activityShortPressed: function(e, view) {
        ...
        if (lastClusterMode == 'Exclusive') {
            var cluster = this.cluster;
            BenomeGlobal.trigger('PopContext', cluster.getView(cluster.focusID), cluster.lastLayoutData.data[cluster.focusID]);
        }
    },

    activityLongPressed: function(e, view) {
        if (this.clusterMode == 'Expanded' || this.clusterMode == 'Normal') {
            this.clusterMode = 'Exclusive';
            this.setClusterExclusive();
        }
        else {
            this.clusterMode = 'Normal';
            this.setClusterCompact();
    ...
    getSurfaceViewClass: function(contextModel) {
        ...
        if (contextModel.get('nodeType') == 'AttributeChoice') {
            return AttributeSurfaces['TestSingleChoice'];
        }
        else {
            return AttributeSurfaces['TestView'];
        }
    },
    ...
    setClusterExclusive: function() {
        var cluster = this.cluster;
        var globalSize = BenomeGlobal.globalSize(),
            refSize = Math.min(globalSize.width, globalSize.height);

        cluster.setRadius(refSize / 2);
        cluster.setConfig({
            hideChildren: true
        });
        cluster.render();

        BenomeGlobal.trigger('PushContext', cluster.getView(cluster.focusID), cluster.lastLayoutData.data[cluster.focusID]);
    },
...
module.exports = ConstructorCluster;